...
```powershell
python agent-eval/test_answers.py --limit 5
```
...
python agent-eval/test_answers.py `
  --input agent-eval/data/question.txt `
  --out agent-eval/output/answers.jsonl
```
...
```powershell
python agent-eval/test_answers.py `
  --base-url http://localhost:8000/chat/api/v1
```

Chạy song song 4 câu một lúc:

```powershell
python agent-eval/test_answers.py --workers 4
```

Script tự giảm số worker theo số câu hỏi. Ví dụ: 1 câu chỉ tạo 1 worker, 5 câu tạo 5 worker, 20 câu tối đa tạo 10 worker.

Chỉ định thư mục output:

```powershell
python agent-eval/test_answers.py `
  --out-dir agent-eval/output/my-run
```

Tăng timeout lên 5 phút:

```powershell
python agent-eval/test_answers.py --timeout 300
```

Xem toàn bộ tùy chọn:

```powershell
python agent-eval/test_answers.py --help
```
